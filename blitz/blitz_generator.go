...
var templates = []SandboxConfigTemplate{
    ...
    // test 321 blitz levels (4)
	{
		name: "Rolling Spheres & Brownians & Yellow BAFs vs Plasma",
		keys: map[SandboxKey]func() int{
	    	key_Asteroids:                  constant(1),
		    key_BAFs:                       constant(2),
	      	key_BlueBAFs:                   constant(1),
	...
	{
      	name: "Rolling Cubes & Orange Motherships vs Plasma",
     	keys: map[SandboxKey]func() int{
     	  	key_Asteroids:                  constant(1),
        	key_BAFs:                       constant(1),
     		key_Brownians:                  constant(1),
    ...
    {
		// Lots of red BAFs, score boxes
    	name: "Red BAFs and score boxes",
    	keys: map[SandboxKey]func() int{
			key_Environment:                randomEnvironement(),
	    	key_RedBAFs:                    constant(2),
			key_Environmentsize:            constant(2),
      		key_Playerspeed:                constant(2),
			key_Scoreboxfrequency:          constant(3),
        	key_Shieldboxfrequency:         constant(2),
	...
	{
		// Multiple enemies, weapon laser, large area
		name: "Multiple enemies vs Laser",
		keys: map[SandboxKey]func() int{
			key_Environment:                randomEnvironement(),
			key_Asteroids:                  constant(1),
			key_BAFs:                       constant(1),
